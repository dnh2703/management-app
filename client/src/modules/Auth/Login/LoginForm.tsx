...
const LoginForm = () => {
  ...
  const login = useGoogleLogin({
    onSuccess: (codeResponse) => {
      setIsLoading(true)
      authApi
        .login({ googleAccessToken: codeResponse.access_token })
        .then((res) => {
          if (res.status === 200) {
            handleLoadUser(res)
            handleLogInResponse(res)
    ...
    <div className='h-full flex items-center'>
      <form onSubmit={handleSubmit(onSubmit)} className='bg-white flex flex-col md:shadow-xl rounded-xl'>
        <div className='p-6 md:p-8 md:pb-0 flex gap-1 flex-col'>
          <h3 className='font-semibold text-zinc-800 text-2xl'>Welcome back!</h3>
          <p className='text-zinc-600 text-sm'>Enter your email below to login your account</p>
        </div>
        ...
        <div className='p-6 md:p-8 md:pt-0 pt-0 flex flex-col'>
          ...
          <p className='text-[13px] pt-2 text-center text-zinc-600'>
            ...
            <Link className='text-blue-500 hover:text-blue-600 font-semibold' to={'/auth/register'}>
              Register
            </Link>
          </p>
